...
#include <string>
#include <map>
#include "tmperrorcodes.hpp"
struct Config
{
	short int						maxClient; 			//!DONE
	std::vector<struct sockaddr_in> sockAddress; 		//
	std::vector<std::string>		indexFiles; 		//!DONE
	bool							listingDirectories; //!DONE
	std::map<e_errorCodes, std::string>		errorPaths; //!NOT SUPPORTED YET

    ...
    public:
        ...
        {           
            ...
            else if (pid == 0)
            {
                ...
                dup2(this->fds[0], STDIN_FILENO); // met le fd 0 a l aplace de in
                dup2(this->fds[0], STDOUT_FILENO); // met le fd 0 a l aplace de out
                close(this->fds[0]);
                close(this->fds[1]);
                // int t[2] = {1,2, NULL}
                // char *args[] = {NULL};
        ...
        void getBody(Client & client)
        {
            ...
            ssize_t ret = recv(this->fds[1], client.messageSend.data(),
                static_cast<std::streamsize>(client.messageSend.size()), 0);
            for (ssize_t i = 0; i < retp; i++)
                std::cout << "." << buff[i];
            close(this->fds[1]);
        };
